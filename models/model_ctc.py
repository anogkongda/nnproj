...
class CTC_Model(nn.Module):
    ...
    def forward(self, x, visualize=False, **kwargs):
...
class Multi_CTC_Model(CTC_Model):
    def __init__(self, add_cnn=False, cnn_param=None, rnn_param=None, num_class=None, drop_out=0.1,dat=0,mme=0, universal=False):
        super(Multi_CTC_Model, self).__init__(add_cnn=add_cnn, cnn_param=cnn_param, rnn_param=rnn_param, num_class=1, drop_out=drop_out)
        batch_norm = rnn_param["batch_norm"]
        rnn_hidden_size = rnn_param["rnn_hidden_size"]
        if batch_norm:
            self.fc_list = nn.ModuleList([nn.Sequential(nn.BatchNorm1d(self.num_directions*rnn_hidden_size),
                                nn.Linear(self.num_directions*rnn_hidden_size, output, bias=False),) for output in num_class])
        else:
            self.fc_list = nn.ModuleList([nn.Linear(self.num_directions*rnn_hidden_size, output, bias=False) for output in num_class])
        self.Lambda = None
        if dat != 0:
            self.grl = GradientReversal(dat)
            self.Lambda = dat
            self.language_c= nn.Sequential(
                nn.Linear(2 * self.num_directions * rnn_hidden_size, rnn_hidden_size),
                nn.ReLU(),
                nn.Linear(rnn_hidden_size, len(num_class))
            )
        self.mme = mme
        if self.mme != 0:
            self.mme_grl = GradientReversal(self.mme)
        self.universal = universal


    def forward(self, x,set_id,visualize=False, mmen_grl=False):
        if self.universal:
            set_id = 0
        if visualize:
            visual = [x]
        ...
        if self.add_cnn:
            ...
            # Shared encoder to here
            if self.Lambda:
                # stats pooling for LID
                mean = torch.mean(x,dim=0)
                std = torch.sqrt(torch.var(x,dim=0) + 1e-8)
                stats = torch.cat((mean,std),dim=-1)
                language = self.language_c(self.grl(stats))
            ...
            if self.Lambda:
                ...
                stats = torch.cat((mean,std),dim=-1)
                language = self.language_c(self.grl(stats))
                language = self.language_c(self.grl(x))
