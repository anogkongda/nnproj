...
class Decoder(object):
    ...
    def wer(self, s1, s2,separator=" "):
        "将空格作为分割计算词错误率"
        b = set(s1.split(separator) + s2.split(separator))
        word2int = dict(zip(b, range(len(b))))

        w1 = [word2int[w] for w in s1.split(separator)]
        w2 = [word2int[w] for w in s2.split(separator)]
...
class BeamDecoder(Decoder):
    ...
    def decode(self, prob_tensor, frame_seq_len=None, n_best=False):
        '''解码函数
        Args:
            prob_tensor   :   网络模型输出
            frame_seq_len :   每一样本的帧数
        ...
        res = self._decoder.decode(probs, frame_seq_len,n_best)
        return res
