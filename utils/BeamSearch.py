...
class ctcBeamSearch(object):
    ...
    def decode(self, inputs, inputs_list,n_best=False):
        '''
        ...
        for batch in range(batches):
            ...
            # sort by probability
            if n_best:
                bestLabelling = last.sort(True)
                res_b = []
                prob = []
                for candidate in bestLabelling:
                    res_b.append(' '.join([self.classes[l] for l in candidate[0]]))
                    prob.append(candidate[1])
                res.append((res_b,prob))
            else:
