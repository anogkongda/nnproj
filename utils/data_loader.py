...
class UnlabelSpeechDataset(Dataset):
    def __init__(self, scp_path, opts):
...
class SpeechDataset(Dataset):
    def __init__(self, vocab, scp_path, lab_path, opts):
        ...
        self.feature_type = opts.feature_type
        self.mel = opts.mel
        has_vocab = True if self.vocab else False
        if opts.feature_type == "waveform":
            ...
            self.process_feature_label(has_vocab)
    
    def process_feature_label(self,has_vocab=True):
        path_dict = []
        #read the ark path
        with open(self.scp_path, 'r') as rf:
            line = rf.readline()
        ...
        with open(self.lab_path, 'r') as rf:
            ...
            while line:
                ...
                if has_vocab:
                    ...
                    label_dict[utt] = [int(c) for c in label.split()]
                line = rf.readline() 
...
def create_input_unsup(batch):
    ...
    for x in range(batch_size):
        ...
        input_sizes[x] = feature_length / inputs_max_length
        utt_list.append(utt)
    return batch_data.float(), input_sizes.float(), utt_list


class UnlabelSpeechDataLoader(DataLoader):
    def __init__(self, *args, **kwargs):
        super(UnlabelSpeechDataLoader, self).__init__(*args, **kwargs)
        self.collate_fn = create_input_unsup
