...
def run_epoch(epoch_id, model, data_iter,loss_fn, device,semi_data_iter=None, optimizer=None, print_every=20, is_training=True, dat=False, mme=False,language_one_hot=False):
    if is_training:
        model.train()
    else:
        model.eval()
...
def main(conf):
    ...
    while not stop_train:
        ...
        train_acc, loss = run_epoch(count, model, train_loader,loss_fn, device,semi_loader,optimizer=optimizer, print_every=opts.verbose_step, is_training=True, dat=dat, mme=mme,language_one_hot=opts.language_one_hot)
        loss_results.append(loss)
        acc, dev_loss = run_epoch(count, model, dev_loader, loss_fn, device, optimizer=None, print_every=opts.verbose_step, is_training=False, dat=dat, mme=mme,language_one_hot=opts.language_one_hot)
